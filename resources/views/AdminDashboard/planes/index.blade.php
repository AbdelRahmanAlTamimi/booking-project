...
@section('content')
    ...
    <div class="col-12">
        <div class="card recent-sales overflow-auto shadow-sm mb-4">
            ...
            <div class="card-body">
                <!-- Planes Filter Form -->
                <form method="GET" action="{{ route('planes.index') }}" class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div>
                        ...
                        <select class="form-select form-select-sm w-auto d-inline-block" name="per-page" onchange="this.form.submit()">
                            ...
                            <option value="{{ $totalPlanes }}" {{ request('per-page') == $totalPlanes ? 'selected' : '' }}>All</option>
                        </select>
                    </div>
                ...
                @if ($planes->isEmpty())
                    <div class="alert alert-info">
                        <strong>No planes found.</strong> <!-- Message when no planes are found -->
                    </div>
                @else
                                ...
                                @foreach ($planes as $plane)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <!-- Edit Button -->
                                                <a href="{{ route('planes.edit', $plane->plane_id) }}" style="width: fit-content; height: fit-content; display: flex; justify-content: center; align-items: center;" class="btn btn-warning btn-sm me-2">
                                                    <i class="fa-regular fa-pen-to-square"></i>
                                                </a>

                                                <!-- Delete Button -->
                                                <form id="delete-form-{{ $plane->plane_id }}" action="{{ route('planes.destroy', $plane->plane_id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button style="width: fit-content; height: fit-content; display: flex; justify-content: center; align-items: center;" type="button" onclick="confirmDelete({{ $plane->plane_id }})" class="btn btn-danger btn-sm">
